#!/usr/bin/env python3
"""
npc_system.py – Rockstar Hybrid v2.6 with Unified Voice

A GTA-like NPC system with a corpus callosum twist: cloud cross-talk (left brain) and VEE/VER offline (right brain).
Uses LLaMA/OpenAI online, VEE offline, and a KnowledgeGraph for memory. Socket-based or cloud comms.
...
class KnowledgeGraph:
    def __init__(self):
        self.nodes = []
        self.edges = []
        self.metadata = {"name": "NPC Brain", "description": "GTA NPC knowledge graph", "version": 1}
...
class Node:
    def __init__(self, id, type, value):
        self.id = id
        self.type = type  # "word", "memory", "event"
        self.value = value

class Edge:
    def __init__(self, id, type, source, target):
        self.id = id
        self.type = type  # "context", "response"
        self.source = source
        self.target = target
...
class NPC:
    ...
    pml_logic: PMLLState = field(default_factory=PMLLState)
...
def cross_talk_dialogue(npc: NPC, player_text: str, world: WorldState, io_socket: IO_Socket) -> tuple[str, str]:
    ...
    npc.knowledge_graph.add_node(Node(len(npc.knowledge_graph.nodes), "memory", reply))
    npc.knowledge_graph.add_edge(Edge(len(npc.knowledge_graph.edges), "response", llama_reply, reply))
...
# VEE/VER Dialogue (Right Brain)
def vee_dialogue(npc: NPC, player_text: str, world: WorldState, io_socket: IO_Socket) -> tuple[str, str]:
    ...
    # Response logic with KnowledgeGraph
    if not reply:
        tokens = text.split()
        for i, token in enumerate(tokens):
            npc.knowledge_graph.add_node(Node(len(npc.knowledge_graph.nodes), "word", token))
            if i > 0:
                npc.knowledge_graph.add_edge(Edge(len(npc.knowledge_graph.edges), "context", tokens[i-1], token))
    ...
    npc.memory_silo.metrics["false_good_rewards"] += 1 if sentiment < 0 else 0
    logger.debug(f"VEE: Iter {npc.iteration_count}, Value {npc.last_value:.2f}")
    return reply, new_state

# Dialogue Engine
def dialogue_engine(npc: NPC, player_text: str, world: WorldState, io_socket: IO_Socket) -> tuple[str, str]:
    if check_internet() and requests and LLAMA_API_KEY and OPENAI_API_KEY:
        return cross_talk_dialogue(npc, player_text, world, io_socket)
    return vee_dialogue(npc, player_text, world, io_socket)
...
# Main Loop
def dialogue_loop(npc: NPC, io_socket: IO_Socket):
    ...
    while True:
        ...
        reply, new_state = dialogue_engine(npc, player_text, world, io_socket)
        npc.state = new_state
...
class TestNPCSystem(unittest.TestCase):
    ...
    def test_dialogue(self):
        reply, state = vee_dialogue(self.npc, "Hey, what’s up?", self.world, self.io_socket)
    ...
    def test_consent(self):
        ...
        vee_dialogue(self.npc, "Consent to flirt", self.world, self.io_socket)
        self.assertIn(self.npc.consent_level, ("flirty", "explicit"))
        vee_dialogue(self.npc, "Stop it", self.world, self.io_socket)
